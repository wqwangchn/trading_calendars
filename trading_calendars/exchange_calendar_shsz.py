...
from zipline.utils.calendars.calendar_holidays_shsz import get_holidays
import warnings
...
start_default = pd.Timestamp('2005-01-05', tz='UTC')
end_base = pd.Timestamp('today', tz='UTC')
end_default = end_base + pd.Timedelta(days=365)

def compute_all_minutes(opens_in_ns, closes_in_ns, lunch_break_start_in_ns, lunch_break_ends_in_ns):
...
class SHSZExchangeCalendar(TradingCalendar):
    ...
    def adhoc_holidays(self):
        return [Timestamp(t, tz=pytz.UTC) for t in get_holidays()]
    ...
    def all_minutes(self):
        ...
        """
        opens_in_ns = self._opens.values.astype('datetime64[ns]').view('int64')
        closes_in_ns = self._closes.values.astype('datetime64[ns]').view('int64')
        lunch_break_start_in_ns = self._lunch_break_starts.values.astype('datetime64[ns]').view('int64')
        lunch_break_ends_in_ns = self._lunch_break_ends.values.astype('datetime64[ns]').view('int64')

        return DatetimeIndex(
            compute_all_minutes(opens_in_ns, closes_in_ns, lunch_break_start_in_ns, lunch_break_ends_in_ns),
            tz='utc',
        )